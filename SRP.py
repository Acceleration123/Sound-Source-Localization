...
class SRP(nn.Module):
    ...
    def forward(self, x):
        ...
        doa_est = self.srp_ula(sig_stft, f_analog)
...
if __name__ == '__main__':
    ...
    rir = np.load('0003_rt60=0.44_d=0.020_r1=2.00_phi1=-55.01_r2=2.00_phi2=78.76.npy')

    sensor_1 = signal.fftconvolve(speech, rir[0][1], mode='same')
    sensor_2 = signal.fftconvolve(speech, rir[1][1], mode='same')
    sensor_3 = signal.fftconvolve(speech, rir[2][1], mode='same')
    sensor_4 = signal.fftconvolve(speech, rir[3][1], mode='same')

    sig = np.stack([sensor_1, sensor_2, sensor_3, sensor_4], axis=1)  # (T, 4)
    sig = torch.tensor(sig).to(device)

    srp_func = SRP(d_inter=0.02)
    doa_est = srp_func(sig)
    print(doa_est)
